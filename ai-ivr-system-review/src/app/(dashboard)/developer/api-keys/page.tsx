    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Secrets are never shown again after creation.</CardDescription>
        </CardHeader>
            ...
            keys.map(key => (
              <div key={key.id} className="rounded-lg border p-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Expires: {key.expiresAt?.toISOString() ?? "Never"} | Last used: {key.lastUsedAt?.toISOString() ?? "Never"}
                </p>
              </div>
            ))
          )}
