        ...
        >
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {isQuickTestCall
                ? `Step 1 of ${getCampaignWizardStepCount("quick-test")} - Internal test setup`
                : `Step 1 of ${getCampaignWizardStepCount("production")} - Audience setup`}
            </p>
          </div>
...
export function getAudienceContinueHref(
  campaignId:
    string,
  ...
  if (
    ...
    return `/communication/campaigns/new/channels?campaign=${encodeURIComponent(
      campaignId
    )}`;
  }

  return `/communication/campaigns/new/knowledge?campaign=${encodeURIComponent(
    campaignId
  )}`;
}

export function getAudienceContinueLabel(
  isQuickTestCall:
    boolean
): string {
  return isQuickTestCall
