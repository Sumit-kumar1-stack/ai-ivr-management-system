...
export async function runCommunicationCampaign(
  ...
  if (
    ...
    log.warn(
      {
        event:
          "communication.worker.duplicate_execution_dropped",

        communicationCampaignId,
      },
      "Communication campaign execution dropped because the campaign is already running"
    );
  ...
  if (
    ...
    throw new Error(
      "Communication campaign tenant not found"
    );
  }
    ...
    await denyExecution({
      ...
      reason:
        error instanceof Error
          ? error.message
          : "Tenant subscription is not available",
    });
  }
  ...
  if (
    ...
    await denyExecution({
      ...
      tenantId,
      reason:
        "Communication campaign is archived or cancelled",
    });
  }

  if (
    billingContext.tenantStatus !==
      TenantStatus.ACTIVE ||
    billingContext.subscription.status !==
  ...
  try {
    await recordAuditEvent({
      ...
      beforeState: {
        ...
        approvalStatus: input.campaign.approvalStatus,
        currentRevision: input.campaign.currentRevision,
        approvedRevision: input.campaign.approvedRevision,
      },
      metadata: {
        approvalStatus: input.campaign.approvalStatus,
        currentRevision: input.campaign.currentRevision,
