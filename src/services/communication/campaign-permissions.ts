...
export function buildCampaignPermissions(
  user: CampaignPermissionUser,
  campaign: CampaignLifecycleSnapshot
): CommunicationCampaignPermissions {
  return {
    canEdit:
      canEditCampaign(
...
export function canDeleteCampaign(
  ...
  return (
    ...
    campaign.status === CommunicationCampaignStatus.DRAFT &&
    (
      hasCapability(user, "CAMPAIGN_DELETE") ||
      (
        hasCapability(user, "CAMPAIGN_EDIT") &&
        (campaign.ownerUserId === user.id || campaign.submittedByUserId === user.id)
      )
    )
  );
}
